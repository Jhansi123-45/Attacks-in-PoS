...
def plot_metrics_comparison(original_metrics, modified_metrics):
    fig, axs = plt.subplots(2, 2, figsize=(12, 12))

    for i, (metric, label) in enumerate([('Accuracy', 'Accuracy Comparison'), ('PR AUC', 'PR AUC Comparison')]):
        ax = axs[i // 2, i % 2]
        ax.bar(original_metrics.keys(), [original_metrics[method][metric] for method in original_metrics.keys()], label='Original Data')
        ax.bar(modified_metrics.keys(), [modified_metrics[method][metric] for method in modified_metrics.keys()], label='Modified Data')
        ax.set_xlabel('Oversampling Technique')
...
plot_metrics_comparison(original_metrics, modified_metrics)
